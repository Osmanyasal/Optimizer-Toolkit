...
ifeq ($(config),debug)
  RESCOMP = windres
  TARGETDIR = bin/Debug
  TARGET = $(TARGETDIR)/libOptimizerToolkitShared.so
  OBJDIR = bin/obj/Debug/OptimizerToolkitShared
  DEFINES += -DOPTKIT_MODE_DEBUG
  INCLUDES += -Isrc
  FORCE_INCLUDE +=
  ALL_CPPFLAGS += $(CPPFLAGS) -MD -MP $(DEFINES) $(INCLUDES)
  ALL_CFLAGS += $(CFLAGS) $(ALL_CPPFLAGS) -m64 -fPIC -g -Wall -O0 -g -fopenmp -msse -march=native -mavx2
  ALL_CXXFLAGS += $(CXXFLAGS) $(ALL_CPPFLAGS) -m64 -fPIC -g -std=c++11 -Wall -O0 -g -fopenmp -msse -march=native -mavx2
  ALL_RESFLAGS += $(RESFLAGS) $(DEFINES) $(INCLUDES)
  LIBS += -lpthread -ldl -ltensorflow -lspdlog
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -shared -Wl,-soname=libOptimizerToolkitShared.so -fopenmp -lpfm
...
ifeq ($(config),release)
  ...
  TARGET = $(TARGETDIR)/libOptimizerToolkitShared.so
  OBJDIR = bin/obj/Release/OptimizerToolkitShared
  DEFINES += -DOPTKIT_MODE_NDEBUG
  INCLUDES += -Isrc
  FORCE_INCLUDE +=
  ALL_CPPFLAGS += $(CPPFLAGS) -MD -MP $(DEFINES) $(INCLUDES)
  ALL_CFLAGS += $(CFLAGS) $(ALL_CPPFLAGS) -m64 -O2 -fPIC -Wall -O2 -fopenmp -msse -march=native -mavx2
  ALL_CXXFLAGS += $(CXXFLAGS) $(ALL_CPPFLAGS) -m64 -O2 -fPIC -std=c++11 -Wall -O2 -fopenmp -msse -march=native -mavx2
  ALL_RESFLAGS += $(RESFLAGS) $(DEFINES) $(INCLUDES)
  LIBS += -lpthread -ldl -ltensorflow -lspdlog
  LDDEPS +=
  ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -shared -Wl,-soname=libOptimizerToolkitShared.so -s -fopenmp -lpfm
  LINKCMD = $(CXX) -o "$@" $(OBJECTS) $(RESOURCES) $(ALL_LDFLAGS) $(LIBS)
  define PREBUILDCMDS
	@echo Running prebuild commands
	@echo [CHECK MSR-SAFE]
...
$(TARGET): $(GCH) ${CUSTOMFILES} $(OBJECTS) $(LDDEPS) $(RESOURCES) | $(TARGETDIR)
	@echo Linking OptimizerToolkitShared
	$(SILENT) $(LINKCMD)
	$(POSTBUILDCMDS)
...
clean:
	@echo Cleaning OptimizerToolkitShared
ifeq (posix,$(SHELLTYPE))
	$(SILENT) rm -f  $(TARGET)
	$(SILENT) rm -rf $(OBJDIR)
else
