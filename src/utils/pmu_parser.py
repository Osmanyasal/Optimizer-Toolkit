...
def process_event_dic(event_dict,event_pe): 
    ...
    result = NAMESPACE_BEGIN.format(vendor_name) + EVENT_CLASS_BEGIN.format(pmu_name)
...
if __name__ == "__main__": 
    ...
    for event_file in event_files:
        if not os.path.isfile(event_file):
            print(f"[ERROR]: The file '{event_file}' does not exists!")
        else:  
            ...
            with open(event_pe[:-3] + EVENT_FILE_EXTENSION,"w") as cpp_file:
