...
namespace optkit::intel{
	enum class ivbep_unc_i : uint64_t {
		...
		UNC_I_ADDRESS_MATCH_MASK_STALL_COUNT = 0x100, // Number of time when it is not possible to merge two conflicting requests
		UNC_I_ADDRESS_MATCH_MASK_MERGE_COUNT = 0x200, // Number of times when two requests to the same address from the same source are received back to back
		UNC_I_CACHE_ACK_PENDING_OCCUPANCY = 0x14, // Write ACK pending occupancy
		UNC_I_CACHE_ACK_PENDING_OCCUPANCY_MASK_ANY = 0x100, // Any source
		UNC_I_CACHE_ACK_PENDING_OCCUPANCY_MASK_SOURCE = 0x200, // Track all requests from any source port
		UNC_I_CACHE_OWN_OCCUPANCY = 0x13, // Outstanding write ownership occupancy
		UNC_I_CACHE_OWN_OCCUPANCY_MASK_ANY = 0x100, // Any source
		UNC_I_CACHE_OWN_OCCUPANCY_MASK_SOURCE = 0x200, // Track all requests from any source port
		UNC_I_CACHE_READ_OCCUPANCY = 0x10, // Outstanding read occupancy
		UNC_I_CACHE_READ_OCCUPANCY_MASK_ANY = 0x100, // Any source
		UNC_I_CACHE_READ_OCCUPANCY_MASK_SOURCE = 0x200, // Track all requests from any source port
		...
		UNC_I_CACHE_WRITE_OCCUPANCY_MASK_ANY = 0x100, // Any source
		UNC_I_CACHE_WRITE_OCCUPANCY_MASK_SOURCE = 0x200, // Track all requests from any source port
		...
		UNC_I_TICKLES_MASK_LOST_OWNERSHIP = 0x100, // Number of request that lost ownership as a result of a tickle
		UNC_I_TICKLES_MASK_TOP_OF_QUEUE = 0x200, // Number of cases when a tickle was received but the request was at the head of the queue in the switch. In this case data is returned rather than releasing ownership
		UNC_I_TRANSACTIONS = 0x15, // Inbound transaction count
		UNC_I_TRANSACTIONS_MASK_READS = 0x100, // Number of read requests (not including read prefetches)
		UNC_I_TRANSACTIONS_MASK_WRITES = 0x200, // Number of write requests. Each write should have a prefetch
		UNC_I_TRANSACTIONS_MASK_RD_PREFETCHES = 0x400, // Number of read prefetches
