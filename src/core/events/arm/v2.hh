...
namespace optkit::arm::v2{
	enum v2 : uint64_t {
		...
		L2D_CACHE = 0x16, // Level 2 data cache accesses
		L2D_CACHE_REFILL = 0x17, // Level 2 data cache refills
		L2D_CACHE_WB = 0x18, // Level 2 data cache write-backs
		BUS_ACCESS = 0x19, // Bus access This event counts for every beat of data transferred over the data channels between the core and the Snoop Control Unit (SCU). If both read and write data beats are transferred on a given cycle
		MEMORY_ERROR = 0x1a, // Local memory error This event counts any correctable or uncorrectable memory error (ECC or parity) in the protected core RAMs
		...
		L3D_CACHE_ALLOCATE = 0x29, // Attributable L3 cache allocation without refill This event counts any full cache line write into the L3 cache which does not cause a linefill
		L3D_CACHE_REFILL = 0x2a, // Attributable L3 cache refill This event counts for any cacheable read transaction returning data from the SCU for which the data source was outside the cluster. Transactions such as ReadUnique are counted as read transactions
		L3D_CACHE = 0x2b, // Attributable L3 cache access This event counts for any cacheable read transaction returning data from the SCU
		...
		L2_TLB_REQ = 0x2f, // Attributable L2 TLB access This event counts on any access to the MMUTC (caused by a refill of any of the L1 TLBs). This event does not count if the MMU is disabled
		L2D_TLB = 0x2f, // Attributable L2 TLB access This event counts on any access to the MMUTC (caused by a refill of any of the L1 TLBs). This event does not count if the MMU is disabled
		...
		TRCEXTOUT0 = 0x4010, // PE Trace Unit external output 0 This event is not exported to the trace unit
		TRCEXTOUT1 = 0x4011, // PE Trace Unit external output 1 This event is not exported to the trace unit
		TRCEXTOUT2 = 0x4012, // PE Trace Unit external output 2 This event is not exported to the trace unit
		TRCEXTOUT3 = 0x4013, // PE Trace Unit external output 3 This event is not exported to the trace unit
		...
		LDST_ALIGN_LAT = 0x4020, // Access with additional latency from alignment
		LD_ALIGN_LAT = 0x4021, // Load with additional latency from alignment
		ST_ALIGN_LAT = 0x4022, // Store with additional latency from alignment
		MEM_ACCESS_CHECKED = 0x4024, // Checked data memory access
		MEM_ACCESS_RD_CHECKED = 0x4025, // Checked data memory access
		MEM_ACCESS_WR_CHECKED = 0x4026, // Checked data memory access
		...
		SVE_PRED_NOT_FULL_SPEC = 0x8079, // SVE predicated operations speculatively executed with a Governing predicate in which at least one element is FALSE
		...
		ASE_SVE_INT8_SPEC = 0x80e3, // Operation counted by ASE_SVE_INT_SPEC where the largest type is 8-bit integer
		ASE_SVE_INT16_SPEC = 0x80e7, // Operation counted by ASE_SVE_INT_SPEC where the largest type is 16-bit integer
		ASE_SVE_INT32_SPEC = 0x80eb, // Operation counted by ASE_SVE_INT_SPEC where the largest type is 32-bit integer
		ASE_SVE_INT64_SPEC = 0x80ef, // Operation counted by ASE_SVE_INT_SPEC where the largest type is 64-bit integer
		
	};
};

namespace v2 = optkit::arm::v2;