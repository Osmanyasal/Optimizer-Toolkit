...
namespace optkit_ultra3plus{
	enum class pe : uint64_t {
		...
		MC_reads_0 = 0x20, // Read requests completed to memory bank 0
		MC_reads_1 = 0x21, // Read requests completed to memory bank 1
		MC_reads_2 = 0x22, // Read requests completed to memory bank 2
		MC_reads_3 = 0x23, // Read requests completed to memory bank 3
		MC_stalls_0 = 0x24, // Clock cycles that requests were stalled in the MCU queues because bank 0 was busy with a previous request
		MC_stalls_2 = 0x25, // Clock cycles that requests were stalled in the MCU queues because bank 2 was busy with a previous request
		MC_writes_0 = 0x20, // Write requests completed to memory bank 0
		MC_writes_1 = 0x21, // Write requests completed to memory bank 1
		MC_writes_2 = 0x22, // Write requests completed to memory bank 2
		MC_writes_3 = 0x23, // Write requests completed to memory bank 3
		MC_stalls_1 = 0x24, // Clock cycles that requests were stalled in the MCU queues because bank 1 was busy with a previous request
		MC_stalls_3 = 0x25, // Clock cycles that requests were stalled in the MCU queues because bank 3 was busy with a previous request
		EC_wb_remote = 0x19, // Counts the retry event when any victimization for which the processor generates an R_WB transaction to non_LPA address region
		EC_miss_local = 0x1a, // Counts any transaction to an LPA for which the processor issues an RTS/RTO/RS transaction
		EC_miss_mtag_remote = 0x1b, // Counts any transaction to an LPA in which the processor is required to generate a retry transaction
		...
		EC_miss_remote = 0x29, // Counts the events triggered whenever the processor generates a remote (R_*) transaction and the address is to a non-LPA portion (remote) of the physical address space
		};};